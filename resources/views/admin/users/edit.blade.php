...
@section('content')
    <div class="page-heading mb-4">
        ...
        <p class="text-muted">Kullanıcının bilgilerini güncelleyin.</p>
    </div>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select name="role" class="form-select" required>
                                ...
                                <option value="super_admin" {{ $user->role == 'super_admin' ? 'selected' : '' }}>Süper Admin
                                </option>
                            </select>
                            @error('role')
                                <span class="text-danger small">{{ $message }}</span>
                            ...
                            <select name="academic_title_id" class="form-select">
                                ...
                                @foreach ($academicTitles as $title)
                                    <option value="{{ $title->id }}"
                                        {{ $user->academic_title_id == $title->id ? 'selected' : '' }}>
